...
const order = await Order.findById(req.params.id); // fetch by id
if (!order) return res.status(404).json({ error: 'Order not found' });
if (order.status === 'pending' && order.customerId.toString() === customerId) {
await Order.findByIdAndDelete(req.params.id); // delete by id
...
router.get('/pending', authenticated, roleCheck(['customer', 'vendor']), async (req, res) => { // allow either role
try {
const id = req.user.userId;
...
if (order.shopId.vendorId.toString() !== req.user.userId) {
